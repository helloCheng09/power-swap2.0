    ...
    <a-layout-header>
      <a-row type="flex">
        ...
        <a-col :span="24" :md="18" class="header-control">
          ...
          <a-dropdown :trigger="['click']" overlay-class-name="header-notifications-dropdown" :get-popup-container="() => wrapper">
            <a-badge count="4">
              <a class="ant-dropdown-link" @click="e => e.preventDefault()">
                <svg-icon class="header_breadc_icon" name="bell" />
              </a>
            </a-badge>

            <a-list slot="overlay" item-layout="horizontal" class="header-notifications-list" :data-source="notificationsData">
              <a-list-item slot="renderItem" slot-scope="item">
                <a-list-item-meta>
                  <template #description>
                    <svg-icon class="header_breadc_icon" name="clock" />
                    <span>{{ item.time }}</span>
                  </template>
          ...
          <a-button ref="secondarySidebarTriggerBtn" type="link" @click="$emit('toggleSettingsDrawer', true)">
            <svg-icon class="header_breadc_icon" name="gear" />
          </a-button>
          <a-button type="link" class="sidebar-toggler" @click="$emit('toggleSidebar', ! sidebarCollapsed) , resizeEventHandler()">
            <svg-icon class="header_breadc_icon" name="menu" />
          </a-button>
          <router-link to="/panel" class="btn-sign-in" @click="e => e.preventDefault()">
            <svg-icon class="header_breadc_icon" name="user" />
            <span>Sign In</span>
          </router-link>
          <!-- / Header Control Buttons -->

          <!-- Header Search Input -->
          <!--          <a-input-search class="header-search" :class="searchLoading ? 'loading' : ''" placeholder="Type here…" :loading="searchLoading" @search="onSearch">-->
          <!--            <template #prefix>-->
          <!--              <svg-icon   class="header_breadc_icon" name="magnifying" />-->
          <!--            </template>-->
          <!--          </a-input-search>-->
          <!-- / Header Search Input -->
